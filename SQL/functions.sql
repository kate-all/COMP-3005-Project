...
SELECT (isbn, title, page_count, price, name)
FROM Book, Publisher
WHERE
        Book.publisher_id = Publisher.publisher_id AND
        Book.isbn = %s
SELECT (isbn, title,  page_count, price, name, num_sold, num_sold_last_month, percent_for_publisher, wholesale_price, in_stock)
FROM Book, Publisher
WHERE
        Book.publisher_id = Publisher.publisher_id AND
        Book.isbn = %s
